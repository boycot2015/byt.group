...
const newsDetailList = (props: any) => {
	...
	const columns: TableColumnType[] = [
		...
		{ title: () => <span>{state.list[0]?.owner || state.list[0]?.artist ? '作者' : '热度'}</span>, dataIndex: 'owner', hidden: window.innerWidth < 768, width: 80, render: (text: string, record: any, index) => <Typography.Paragraph key={text || index} ellipsis={{ tooltip: text || record.artist || record.hot, rows: 2 }}>{text || record.hot || '--'}</Typography.Paragraph>, align: 'left' },
		...
		{ title: '详情', hidden: window.innerWidth < 768, dataIndex: 'desc', width: 300, render: (text: string, record: any) => <Typography.Paragraph key={text} ellipsis={{ tooltip: text, rows: 2 }}>{text || '--'}</Typography.Paragraph>, align: 'left' },
	]
	...
	return <ConfigProvider locale={zhCN}>
		...
		<Table
			...
			scroll={{ x: '320px', y: window.innerWidth > 768 ? 'calc(100vh - 350px)' : 'calc(100vh - 210px' }}
			pagination={window.innerWidth < 768 ? false : undefined}
